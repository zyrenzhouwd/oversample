...
def plot_predictions(x_maj, x_min, x_os=None):
    ...
    x0s = np.linspace(x_1, x_2, int((x_2 - x_1) // 0.01))
    x1s = np.linspace(y_1, y_2, int((y_2 - y_1) // 0.01))
    ...
    if x_os is None:
        plt.contourf(x0, x1, y_pred, cmap=plt.cm.gist_rainbow, alpha=0.1)
    else:
...
def plot_data_all_test(x_maj, x_min, x_os, dataset=None, model=None):
    ...
    for f1 in range(0, x_maj.shape[1]):
        for f2 in range(f1 + 1, x_maj.shape[1]):
            ...
            plt.scatter(x_os[:, f1], x_os[:, f2], marker='.', c='#00cb0a', s=3, label='New')
...
def plot_data(x_maj, x_min, x_os, dataset=None, model=None):
    ...
    plt.scatter(x_maj[:, f1], x_maj[:, f2], marker='o', c='blue', s=3, label='Majority')
    plt.scatter(x_min[:, f1], x_min[:, f2], marker='o', c='red', s=3, label='Majority')
    plt.scatter(x_os[:, f1], x_os[:, f2], marker='.', c='#00cb0a', s=3, label='New')
    # plt.tick_params(labelbottom=False, labelleft=False)
    plt.legend(loc='upper right')
...
if __name__ == '__main__':
    from data_process import process_yeast

    maj_limit, min_limit = 400, 40
    ...
    x_os, y_os = oversampling('crossover', np.concatenate([x_maj, x_min], axis=0), np.concatenate([y_maj, y_min], axis=0), row_1=400)
    # plot_predictions(x_maj, x_min, x_os)
    # plot_data(x_maj, x_min, x_os, 'yeast', 'crossover')
    plot_data_all_test(x_maj, x_min, x_os, 'yeast', 'crossover')
